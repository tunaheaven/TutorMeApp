...
{
    NSString* link = @"http://localhost/Test%20Server/wwwroot/include_php/registerData.php";
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    {
        result = nil;
        [[self delegate] isRegisterSuccessful:NO];
    }
    ...
    {
        result = nil;
        [[self delegate] isRegisterSuccessful:YES];
    }
}
...
- (NSString *) parseJSON: (NSData *) data ;
{
    ...
    {
        result =  (NSString*)[json objectForKey:@"register"];
        if ([result caseInsensitiveCompare:@"failed" ] == NSOrderedSame) //failed register
        {
            registerError = [[NSMutableString alloc] initWithString:(NSString*) [json objectForKey:@"error"]];
            // registerError = (NSString*) [json objectForKey:@"error"];
            NSLog(@"parse JSON at RegisterConnection %@", registerError);
        }
        else registerError = nil;
    }
